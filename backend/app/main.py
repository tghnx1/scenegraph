...
def create_bootstrap_user(connection: Connection) -> None:
    if (
        ...
        return
    if BOOTSTRAP_USER_ROLE not in {"user", "agent"}:
        raise RuntimeError("BOOTSTRAP_USER_ROLE must be user or agent")
...
async def register(
    ...
    if register_data.role not in {"user", "agent"}:
        return RegisterResponse(
            success=False,
...
async def change_user_role(
    ...
    if role_data.role not in {"user", "agent"}:
