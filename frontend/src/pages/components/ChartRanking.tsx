  ...
  return (
    ...
    >
      {entries.map((entry, index) => {
        ...
        return (
          <li className="grid grid-cols-[1rem_minmax(0,1fr)_6ch] items-center gap-2" key={entry.item.id ?? `${entry.label}-${index}`}>
            ...
            <strong className="whitespace-nowrap text-right text-xs tabular-nums text-[var(--text)]">{formatValue(entry.value)}</strong>
          </li>
        )
      })}
    </ol>
